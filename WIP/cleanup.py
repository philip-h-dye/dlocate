...
def load_config(app_name):
    ...
    _module.Config  = namedtuple('Config',['app_name','app_exec','drives'
                                ,'aliasdb'] +defaults.keys()+ctx['options'])

    _module.Drive   = namedtuple('Drive',['name','root','alias']
                                 + defaults.keys()+ctx['options'] )

    data = load( stream = file( os.path.expanduser(config_file), 'r')
                 , Loader=Loader )
...
def drive_options ( config, drive ) :
    ...
    for key in ['root','name','alias'] + defaults.keys() :
...
if __name__ == '__main__' :
    ...
    def load_and_print(app):
        print("dlocate.load_config('{}') :".format(app))
        config = load_config('updatedb')
        print (config)
        print ('')
        print ('')

    load_and_print('updatedb')

    load_and_print('locate')
